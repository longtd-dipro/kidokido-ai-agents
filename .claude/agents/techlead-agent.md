...
### 2. Query semantic memory để xác định file thực tế
```bash
...
### 3. Blast radius check (BẮT BUỘC cho mọi file dự kiến sửa/thêm interface)
```bash
...
## Bước tiếp theo
Sau khi tạo xong task(s) → "Hãy là Dev Agent, implement task: `docs/features/<feature-slug>/tasks/task-N.md`".
